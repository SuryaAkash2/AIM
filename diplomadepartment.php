...
<html>
<head>
  <title>AITAM DIPLOMA DEPARTMENT</title>
...
<div class="row">
  ...
  padding: 0.5em 1em;
  width:19%; margin-left: 39%;
  font-size: 1.25em;
  text-align: center;">DIPLOMA DEPARTMENT</h1><hr>
    </div>
...
<form action="diplomaentry.php" method="POST">
...
<th>BRANCH</th>
 ...
 <td>
    ...
    <td>
       <select name="block">
       <option>Bblock</option>
      <option>Ablock</option>  
      <option>Fblock</option> 
      <option>Cblock</option>
      <option>Dblock</option>
      <option>Eblock</option>
...
<table class="table table-hover table-bordered results"  id="ticketLinesTable" >
  <thead>
     <h6 style="margin-top:2% !important;"><a href="mechexcelcon.php">Download Data</a></h6><br>
    <tr>
      <th>Sno</th>
      <th>DATE</th>
      <th>INFRASTRUCTURE</th>
  ...
  <tbody >
     <?PHP
              $i=1;
             include 'connect.php';
            $oquery = "SELECT * FROM `branch` WHERE  `department`='DIPLOMA'  ORDER BY `sno` DESC";
            $sql = mysqli_query($conn, $oquery);
              while($row = mysqli_fetch_array($sql)){
               ...
               echo " <tr><th>".$i."</th><td>".$row['date']."</td><td>".$row['infra']."</td>
                        ...
                        <td>".$row['purpose']."</td><td>".$row['temper']."</td><td>".$row['status']."</td><td><a href='diplomaremove.php?k=".$row['sno']."'><b>Remove</b></a></tr>";
                        $i++;
              }
         ?>
  </tbody>
