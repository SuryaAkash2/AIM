...
<html>
<head>
  <title>AITAM EEE DEPARTMENT</title>
...
<div class="row">
  ...
  font-size: 1.25em;
  text-align: center;">EEE DEPARTMENT</h1><hr>
    </div>
...
<form action="eeeentry.php" method="POST">
...
<table class="table table-hover table-bordered results"  id="ticketLinesTable" >
  <thead>
     <h6 style="margin-top:2% !important;"><a href="eeeexcelcon.php">Download Data</a></h6><br>
    <tr>
      <th>Sno</th>
      <th>DATE</th>
      <th>INFRASTRUCTURE</th>
  ...
  <tbody >
     <?PHP
              $i=1;
             include 'connect.php';
            $oquery = "SELECT * FROM `branch` WHERE  `department`='EEE'  ORDER BY `sno` DESC";
            $sql = mysqli_query($conn, $oquery);
              while($row = mysqli_fetch_array($sql)){
               ...
               echo " <tr><th>".$i."</th><td>".$row['date']."</td><td>".$row['infra']."</td>
                        ...
                        <td>".$row['purpose']."</td><td>".$row['temper']."</td><td>".$row['status']."</td><td><a href='eeeremove.php?k=".$row['sno']."'><b>Remove</b></a></tr>";
                        $i++;
              }
         ?>
  </tbody>
