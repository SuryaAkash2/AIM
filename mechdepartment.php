...
<html>
<head>
  <title>AITAM MECH DEPARTMENT</title>
...
<div class="row">
  ...
  font-size: 1.25em;
  text-align: center;">MECHANICAL DEPARTMENT</h1><hr>
    </div>
...
<form action="mechentry.php" method="POST">
...
<table class="table table-hover table-bordered results"  id="ticketLinesTable" >
  ...
  <tbody >
     <?PHP
              $i=1;
             include 'connect.php';
            $oquery = "SELECT * FROM `branch` WHERE  `department`='MECH'  ORDER BY `sno` DESC";
            $sql = mysqli_query($conn, $oquery);
              while($row = mysqli_fetch_array($sql)){
               ...
               echo " <tr><th>".$i."</th><td>".$row['date']."</td><td>".$row['infra']."</td>
                        ...
                        <td>".$row['purpose']."</td><td>".$row['temper']."</td><td>".$row['status']."</td><td><a href='mechremove.php?k=".$row['sno']."'><b>Remove</b></a></tr>";
                        $i++;
              }
         ?>
  </tbody>
