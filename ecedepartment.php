...
<html>
<head>
  <title>AITAM ECE DEPARTMENT</title>
...
<div class="row">
  ...
  font-size: 1.25em;
  text-align: center;">ECE DEPARTMENT</h1><hr>
    </div>
...
<form action="eceentry.php" method="POST">
...
<table class="table table-hover table-bordered results"  id="ticketLinesTable" >
  <thead>
     <h6 style="margin-top:2% !important;"><a href="eceexcelcon.php">Download Data</a></h6><br>
    <tr>
      <th>Sno</th>
      <th>DATE</th>
      <th>INFRASTRUCTURE</th>
  ...
  <tbody >
     <?PHP
              $i=1;
             include 'connect.php';
            $oquery = "SELECT * FROM `branch` WHERE  `department`='ECE'  ORDER BY `sno` DESC";
            $sql = mysqli_query($conn, $oquery);
              while($row = mysqli_fetch_array($sql)){
               ...
               echo " <tr><th>".$i."</th><td>".$row['date']."</td><td>".$row['infra']."</td>
                        ...
                        <td>".$row['purpose']."</td><td>".$row['temper']."</td><td>".$row['status']."</td><td><a href='eceremove.php?k=".$row['sno']."'><b>Remove</b></a></tr>";
                        $i++;
              }
         ?>
  </tbody>
